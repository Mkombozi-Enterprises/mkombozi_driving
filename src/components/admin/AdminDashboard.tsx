...
const SECTIONS = [
  ...
  { id: "founders", label: "Founders & yard" },
  { id: "copy", label: "Hero & about copy" },
  { id: "nav", label: "Nav & journey" },
  { id: "route", label: "Licence route stops" },
  ...
  { id: "wall", label: "Wall of passes" },
  { id: "form", label: "Enquiry form options" },
] as const;
...
export function AdminDashboard({ initial }: { initial: SiteContent }) {
  ...
  const updatedLabel = useMemo(
    () =>
      content.updatedAt
        ? new Date(content.updatedAt).toLocaleString("en-KE")
        : "—",
    [content.updatedAt]
  );
  ...
  const save = useCallback(() => {
    ...
    startTransition(async () => {
      ...
      setStatus({ type: "ok", msg: "Saved. Public site will refresh on next load." });
      router.refresh();
    });
  }, [content, router]);
    ...
    <div className="cms">
      <div className="cms-shell">
        <aside className="cms-nav">
          <h1>Mkombozi CMS</h1>
          <p className="meta">
            File store · <code>content/site.json</code>
            <br />
            Last saved: {updatedLabel}
            <br />v{content.version}
          </p>
          {SECTIONS.map((s) => (
            <button
              key={s.id}
          ...
          <hr style={{ borderColor: "#2a2f3a", margin: "16px 0" }} />
          <a
            href="/"
            style={{ color: "#9a9da6", fontSize: "0.85rem", padding: "8px 12px" }}
          >
            ← View public site
          </a>
        ...
        <main className="cms-main">
          ...
          {status ? (
            <div className={`cms-banner ${status.type === "ok" ? "ok" : "err"}`}>
              {status.msg}
            </div>
          ) : null}

          {section === "site" && (
            <SiteFields content={content} setContent={setContent} />
          )}
          {section === "founders" && (
            <FoundersFields content={content} setContent={setContent} />
          )}
          {section === "copy" && (
            <CopyFields content={content} setContent={setContent} />
          )}
          {section === "nav" && (
            <NavFields content={content} setContent={setContent} />
          )}
          {section === "route" && (
            <RouteFields content={content} setContent={setContent} />
          )}
          {section === "courses" && (
            <CoursesFields content={content} setContent={setContent} />
          )}
          {section === "addons" && (
            <AddOnsFields content={content} setContent={setContent} />
          )}
          {section === "packages" && (
          ...
          {section === "faqs" && (
            <FaqFields content={content} setContent={setContent} />
          )}
          {section === "fleet" && (
            <FleetFields content={content} setContent={setContent} />
          )}
          {section === "wall" && (
            <WallFields content={content} setContent={setContent} />
          )}
          {section === "form" && (
            <FormOptionsFields content={content} setContent={setContent} />
          )}
        </main>
      </div>
    </div>
...
function SiteFields({ content, setContent }: EditorProps) {
...
function FoundersFields({ content, setContent }: EditorProps) {
  return (
    <>
      <div className="cms-card">
        <h3>Founder quote</h3>
...
function CopyFields({ content, setContent }: EditorProps) {
  const s = content.site;
  return (
    <div className="cms-card">
...
function NavFields({ content, setContent }: EditorProps) {
  return (
    <>
      <div className="cms-card">
...
function RouteFields({ content, setContent }: EditorProps) {
  return (
    <div className="cms-card">
...
function CoursesFields({ content, setContent }: EditorProps) {
  return (
    <>
...
function AddOnsFields({ content, setContent }: EditorProps) {
  const add = () => {
    const item: AddOn = {
      tag: "Add-On",
      title: "New add-on",
...
function PackageFields({ content, setContent }: EditorProps) {
...
function InstructorsFields({ content, setContent }: EditorProps) {
  ...
  return (
    <div className="cms-card">
      ...
      <p style={{ color: "#9a9da6", fontSize: "0.85rem", marginTop: 0 }}>
        Photo upload stores files in Supabase Storage (<code>cms-media</code>).
        Then click <strong>Save changes</strong> to persist the instructor record.
      </p>
...
function FaqFields({ content, setContent }: EditorProps) {
  const add = () => {
    const item: FaqItem = {
      id: uid("faq"),
      q: "New question?",
...
function FleetFields({ content, setContent }: EditorProps) {
  return (
    <div className="cms-card">
      <h3>Fleet cards</h3>
...
function WallFields({ content, setContent }: EditorProps) {
  const add = () => {
    const item: WallPass = {
      id: uid("pass"),
      name: "Graduate",
...
function FormOptionsFields({ content, setContent }: EditorProps) {
  return (
    <div className="cms-card">
